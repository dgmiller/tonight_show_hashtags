##A filter that will filter out any retweets, also used as an example for
...
# this function recieves a dataset object, which is normally the representation
# of all the tweets under one hashtag
# with filters it will just be a single tweet as opposed to all of them
def filter(dset) :
    ...
    # you can use any label you see on the website under the second columns 
    # even the ones you made yourself
    # in the case of filters the list returned will always only have one element in it
